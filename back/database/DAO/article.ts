...
export const insert_article =(article:Article): Promise<RowDataPacket[]>=>{
   return new Promise((resolve, reject) => {
    const sql = 'INSERT INTO articles (title, content, summary, cover_image, status) VALUES (?, ?, ?, ?, ?)'
    db.query(sql, [article.title, article.content, article.summary, article.cover_image, article.status], (err, result:RowDataPacket[]) => {
      if (err) {
        reject(err)
      } else {
        resolve(result)
...
export const select_article =(page:number,limit:number,keyword:string,status:ArticleStatus): Promise<RowDataPacket[]>=>{
   return new Promise((resolve, reject) => {
    ...
    const fields = 'SELECT id, title, summary, cover_image, view_count, created_at'
